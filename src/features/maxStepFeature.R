getMaxStepFeatures <- function(eegData, name) {
  ...
  features <- matrix(nrow=1, ncol=nofTimeIntervals*3)
  names <- vector(mode='character', length=nofTimeIntervals*3)
  ...
  for(i in 1:nofTimeIntervals) {
    ...
    features[1,i*3-2] <- max(validSteps)
    names[i*3-2] <- paste('MaxStep',name,time_intervals_ms[i],'ms',sep="")
